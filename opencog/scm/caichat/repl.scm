...
(define (caichat-help)
  ...
  (display "\nSession Persistence and Mediation:\n")
  (display "  (caichat-create-persistent-session name provider model)\n")
  (display "                                         - Create a named persistent session\n")
  (display "  (caichat-resume-session name provider model)\n")
  (display "                                         - Resume a saved persistent session\n")
  (display "  (caichat-mediate-session session-id)   - Sync session state to hypergraph/file\n")
  (display "  (caichat-audit-core-modules)           - Audit modules for hypergraph synergy\n")
  (display "\nIn REPL mode:\n")
  ...
  (display "  /persist <name> - Create a persistent session\n")
  (display "  /resume <name> - Resume a persistent session\n")
  (display "  /mediate  - Sync current session to persistent storage\n")
  ...
  (let ((parts (string-split line #\space)))
    (let ((command (car parts))
      ...
      (case (string->symbol (substring command 1))
        ...
        ((persist)
         (if (null? args)
             (display "Usage: /persist <session-name>\n")
             (let* ((name (car args))
                    (session-id (caichat-create-persistent-session
                                  name current-provider current-model)))
               (set! current-session session-id)
               (format #t "Created persistent session '~a' (id: ~a)\n" name session-id))))
        ((resume)
         (if (null? args)
             (display "Usage: /resume <session-name>\n")
             (let* ((name (car args))
                    (session-id (caichat-resume-session
                                  name current-provider current-model)))
               (set! current-session session-id)
               (format #t "Resumed persistent session '~a' (id: ~a)\n" name session-id))))
        ((mediate)
         (if current-session
             (begin
               (caichat-mediate-session current-session)
               (display "Session state synced to persistent storage.\n"))
             (display "No active session to mediate.\n")))
        ((provider)
         (if (null? args)
